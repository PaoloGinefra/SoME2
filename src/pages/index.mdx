...
After a while — maybe hours, maybe days — he shouted in pure elation: '*EUREKA!!!* I need to follow <Spoiler>ULUURU</Spoiler>'.
...
Anyway, using the suggested algorithm the sequence is soon found: "<Spoiler>GRRGR</Spoiler>"
...
Getting back to the scenarios we encountered, to get a directed graph we can work on out of them, we’ll number the crossroads starting from the bottom of the mine, while for the lego bricks we’ll lay out the four orientations. Drawing the transitions according to the mine’s configuration and to the physics of the collisions of the roofs with the bricks we obtain:
    ...
    <Caption>
      Tap on the button to switch between the two scenarios
    </Caption>
    ...
    />
    <Caption>
      Enter a word made of Gs and Rs for the Conveyor Belt or made of Us, Rs, Ls and Ds for the Mine, select a starting state and compute the result. Then follow it on the graph.
    </Caption>
...
Let's start by noticing that when a graph allows a synching word to exist, it automatically implies that there are infinitely many of them.
...
The core idea is not bad, in fact, it's the one behind many synching word-finding algorithms. But it needs quite a bit of polish.
...
Finally applying all the transitions to the resulting sets of the previous step
Iteratively, until we were to find a set with just one state.
    ...
    />
     ...
     <Caption>
      Enter a word and check if it's a synching one following the automabots
    </Caption>
This way a tree can be built.
...
If you think about it, we are building a new automaton/directed graph where each state is a set of states. Finding the shortest synchronising word has become a matter of finding the shortest path between two nodes of a graph. Pretty good solutions to this problems are already known: Algorithms like Dijkstra’s or A* do the job fairly well.
    ...
    />
    <Caption>
      Now that we know how to compute the synchronising word of an automaton we can have some fun modifying the mine. Remember you can not create unaccessible areas
    </Caption>
    <TextContainer>
...
<Quote>It’s definitely something but we are talking about a potential absolute worst case: an exponential bound is not that exciting, quite the opposite, actually. Also, every example of automaton I have encountered so far has a synch word much shorter than that</Quote>
...
<FadeBg color="#3b2159">
  <TextContainer>
    ...
    We are a team of 5 software engineering students (*names and roles are in alphabetical order*):
    - Dario Crosa: <Credit>Creative Coding</Credit> <Credit>Full stack Web Development</Credit>
    - Federico Grandi: <Credit>Full stack Web Development</Credit>
    - Francesco Genovese: <Credit>Frontend Web Development</Credit> <Credit>Writing</Credit>
    - Matteo Garzone: <Credit>Creative Coding</Credit>
    - Paolo Ginefra: <Credit>Creative Coding</Credit> <Credit>Pixel Art </Credit> <Credit>Project Management</Credit> <Credit>Writing</Credit>
...
If you are interested in the code behind this article it’s available on <a href="https://github.com/PaoloGinefra/SoME2" style={{color: 'white'}} target="blank">Github</a>

...
/>
<Controls
  actions={['avraham-move-step', 'avraham-show-map', 'zoom', 'zoom-reset']}
/>
...
- How many instructions would the smallest one contain at best? and at worst? <span id="AvrahamMazeExperience"> </span>

<SketchIframe
  src="/sketches/MazeExperience1/index.html?auto=true"
  initialHeigth={500}
/>
<Caption>Now Avraham will only stop at crossroads</Caption>
<Controls
  actions={[
...
After a while — maybe hours, maybe days — he shouted in pure elation:
...
But it doesn’t end there: a series of poorly made life decisions (some of them regarding hibernation) has led you to be in charge of designing the contraptions for the LEGO car factory.
...
<Caption>
<span className="emoji">🖱️</span> Click on the Lego barriers to switch their position to rotate the car roofs in the right orientation.

<span className="emoji">➕</span> You can remove or add barriers clicking on the
<kbd>-</kbd> and <kbd>+</kbd> buttons.
...
With a quick browser search – using your mind, obviously – you find an old paper that seems to give you all the answers.
...
In both the examples above it is clear that after reaching a state you are presented with a choice, you could either go Up, Down, Left or Right in the mine or choose the Green rather than the Red brick for the legos. Let’s identify transitions with symbols like U, D, L, R or G and R or 0, 1, 2, 3 and so on.
Now, think of the last sequence of symbols you used to convey some sort of meaning: a <Spoiler> **Word** </Spoiler>. That's what we’ll call a sequence of instructions in our model.
...
- As for the conveyor belt, we’ll draw the four orientations as the nodes and we’ll use red/green arrows to represent a collision with a red/green brick;
- As for the mine, we’ll number the crossroads starting from the bottom left corner proceeding to the top right, row by row. Then we’ll use red, blue, green or orange arrows to represent the choice to go up, right, down or left. For reference, the numbering is also drawn on the mine’s map [here](#AvrahamMazeExperience).
...
Our setups are officially represented as a directed graph:
    ...
    <Caption>
      <span className="emoji">🔘</span> Tap on the button to switch between the two scenarios.
      <PcTip>Scroll into the frame to zoom in and out. Press <kbd>R</kbd> to reset the applet's zoom</PcTip>
    ...
    />
    <Caption>
      <span className="emoji">⌨️</span> Enter a word made of Gs and Rs for the Conveyor Belt or made of Us, Rs, Ls and Ds for the Mine.

       <span className="emoji">👉</span> Select a starting state and compute the result and follow it on the graph.
       <PcTip>Scroll into the frame to zoom in and out. Press <kbd>R</kbd> to reset the applet's zoom</PcTip>
...
Wow a lot of “edge”, “node” and “colour” in that sentence. Don't panic, it's basically what we did before assigning a symbol for each possible transition and then assigning a colour for every symbol.
...
1. A synchronising word needs to work starting from any vertex of the graph so, naturally, each symbol must be interpretable at each node.
   Let's say your symbols are A, B and C. If there was a node with just two edges coming out of it and we assigned to the edges the letters A and B, what would happen if at that node we asked for C? It wouldn’t make any sense, would it? The same goes for a node
   ...
   <SketchIframe src="/sketches/Condition1/index.html" initialHeigth={300} />
...
2. A synchronising word leads every node to a single one. So, first of all, said node must be reachable, in one way or another, from every node of the graph. Remember that a graph is just a set of nodes and edges so there's nothing against building one with edgeless nodes or isolated parts. To protect ourselves from those, we need to be able to reach every node starting from every other node via a valid route, in other _words_, we need a **strongly connected graph**
   ...
   <SketchIframe src="/sketches/Condition2/index.html" initialHeigth={300} />
...
3. Unfortunately, now it gets a little more complicated. Picture a graph with just three nodes with an out-degree of 1, a graph where each node points to the one which does not point to it. You get a cycle, which is not that big of a deal per se, but this cycle is special. You'll never be able to make two different nodes collapse into one using a word. It's impossible.
   ...
   <SketchIframe src="/sketches/Condition3/index.html" initialHeigth={300} />
...
They simply conjectured that… you guessed it: as long as those conditions are met,
...
Finally, we would apply all the transitions to the resulting sets of the previous step iteratively, until we were to find a set with just one state, finally syncronised.
    ...
    />
     ...
     <Caption>
      <span className="emoji">🔃</span> Enter a word and check if it's a synchronising one following the automabots.

      <span className="emoji">🔘</span> You can switch between the two scenarios clicking on the bottom button.
      <PcTip>Scroll into the frame to zoom in and out. Press <kbd>R</kbd> to reset the applet's zoom</PcTip>
...
If you think about it, we are building a new automaton/directed graph where each state is a set of states. Finding the shortest synchronising word has become a matter of finding the shortest path between two nodes of a graph. Pretty good solutions to this problems are already known: Algorithms like Dijkstra’s or A\* do the job fairly well.
    ...
    />
    <Caption>
      <span className="emoji">🚧</span> Now that we know how to compute the synchronising word of an automaton we can have some fun modifying the mine.

      <span className="emoji">⚙️</span> The shortest synchronising word will appear on top of the mine's map

      <span className="emoji">❗</span> Remember you can not create unaccessible areas!

      <span className="emoji">🧭</span> You can move Avrahm just like before
      <PcTip>Scroll into the frame to zoom in and out. Press <kbd>R</kbd> to reset the applet's zoom</PcTip>
...
<Quote>
  It’s definitely something but we are talking about a potential absolute worst
...
Most of the things in the applets are procedurally generated, if you are interested in the code behind this article it’s available on <a href="https://github.com/PaoloGinefra/SoME2" target="blank">Github</a>

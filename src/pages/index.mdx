...
<Caption>
  ...
  <span >🗺️</span> Click on Avraham to show the mine's map.
...
<Caption>
<span >🖱️</span> Click on the Lego barriers to switch their position to rotate the car roofs in the right orientation.

<span>➕</span> You can remove or add barriers clicking ond the <kbd>-</kbd> and <kbd>
  +
</kbd> buttons.

<span >🔛</span> Once some barriers are added you can scroll horizontally to view them all
</Caption>
<TextContainer>
...
Our setups are officially represented as a directed graph:
    ...
    <Caption>
      <span >🔘</span> Tap on the button to switch between the two scenarios.
      <PcTip>Scroll into the frame to zoom in and out</PcTip>
    ...
    />
    <Caption>
      <span >⌨️</span> Enter a word made of Gs and Rs for the Conveyor Belt or made of Us, Rs, Ls and Ds for the Mine.

       <span >👉</span> Select a starting state and compute the result and follow it on the graph.
       <PcTip>Scroll into the frame to zoom in and out</PcTip>
...
Finally applying all the transitions to the resulting sets of the previous step iteratively, until we were to find a set with just one state.
    ...
    />
     ...
     <Caption>
      <span >🔃</span> Enter a word and check if it's a synchronising one following the automabots.

      <span >🔘</span> You can switch between the two scenarios clicking on the bottom button.
      <PcTip>Scroll into the frame to zoom in and out</PcTip>
...
If you think about it, we are building a new automaton/directed graph where each state is a set of states. Finding the shortest synchronising word has become a matter of finding the shortest path between two nodes of a graph. Pretty good solutions to this problems are already known: Algorithms like Dijkstra’s or A\* do the job fairly well.
    ...
    />
    <Caption>
      <span >🚧</span> Now that we know how to compute the synchronising word of an automaton we can have some fun modifying the mine.

      <span >⚙️</span> The shortest synchronising word will appear on top of the mine's map

      <span >❗</span> Remember you can not create unaccessible areas!

      <span >🧭</span> You can move Avrahm just like before
      <PcTip>Scroll into the frame to zoom in and out</PcTip>

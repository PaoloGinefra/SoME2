...
<Caption>
  ...
  <span class="emoji">🗺️</span> Click on Avraham to show the mine's map.
...
<Caption>
<span class="emoji">🖱️</span> Click on the Lego barriers to switch their position to rotate the car roofs in the right orientation. 
...
Getting back to the scenarios we encountered, to get a directed graph we can work on out of them, we’ll number the crossroads starting from the bottom of the mine, while for the lego bricks we’ll lay out the four orientations. Drawing the transitions according to the mine’s configuration and to the physics of the collisions of the roofs with the bricks we obtain:
    ...
    <Caption>
      <span class="emoji">🔘</span> Tap on the button to switch between the two scenarios.
      <PcTip>Scroll into the frame to zoom in and out</PcTip>
    ...
    />
    <Caption>
      <span class="emoji">⌨️</span> Enter a word made of Gs and Rs for the Conveyor Belt or made of Us, Rs, Ls and Ds for the Mine.
      
       <span class="emoji">👉</span> Select a starting state and compute the result and follow it on the graph.
    </Caption>
...
Iteratively, until we were to find a set with just one state.
    ...
    />
     ...
     <Caption>
      <span class="emoji">🔃</span> Enter a word and check if it's a synching one following the automabots. 
      
      <span class="emoji">🔘</span> You can switch between the two scenarios clicking on the bottom button.
      <PcTip>Scroll into the frame to zoom in and out</PcTip>
...
If you think about it, we are building a new automaton/directed graph where each state is a set of states. Finding the shortest synchronising word has become a matter of finding the shortest path between two nodes of a graph. Pretty good solutions to this problems are already known: Algorithms like Dijkstra’s or A\* do the job fairly well.
    ...
    />
    <Caption>
      <span class="emoji">🚧</span> Now that we know how to compute the synchronising word of an automaton we can have some fun modifying the mine. 
      
      <span class="emoji">❗</span> Remember you can not create unaccessible areas!
      <PcTip>Scroll into the frame to zoom in and out</PcTip>

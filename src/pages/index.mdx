...
Getting back to the scenarios we encountered: - As for the conveyor belt, we’ll draw the four orientations as the nodes and we’ll use red/green arrows to represent a collision with a red/green brick; - As for the mine, we’ll number the crossroads starting from the bottom left corner proceeding to the top right, row by row. Then we’ll use red, blue, green or orange arrows to represent the choice to go up, right, down or left. For reference, the numbering is also drawn on the mine’s map [here](#AvrahamMazeExperience).

...
If you think about it, we are building a new automaton/directed graph where each state is a set of states. Finding the shortest synchronising word has become a matter of finding the shortest path between two nodes of a graph. Pretty good solutions to this problems are already known: Algorithms like Dijkstra’s or A\* do the job fairly well.
    ...
    <TextContainer>
      <Caption>
        Now that we know how to compute the synchronising word of an automaton we can have some fun modifying the mine.<br />
        The shortest synchronising word will appear on top of the mine's map. <br />After the arrow you can see the synchronisation spot.<br />
        Remember you can not create unaccessible areas!<br />
      </Caption>
      <Controls
        actions={[
          'avraham-move-crossroads',

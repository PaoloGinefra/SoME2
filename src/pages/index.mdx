...
You can remove or add barriers clicking ond the <kbd>-</kbd> and <kbd>+</kbd> buttons.
...
Anyways, using the suggested algorithm the sequence is soon found: <Spoiler> GRRGR </Spoiler>. You place the barriers in the figured order: G meaning Green and R meaning Red.
...
Iteratively, until we were to find a set with just one state.
    ...
    />
     ...
     <Caption>
      <span className="emoji">🔃</span> Enter a word and check if it's a synching one following the automabots.

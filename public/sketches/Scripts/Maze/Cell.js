function Cell(i, j) {
  ...
  this.getNeighbors = function (selectedDirection = undefined) {
    let neighbors = []
    if (grid[this.i][this.j - 1]) neighbors[Top] = grid[this.i][this.j - 1]
    if (grid[this.i + 1] && grid[this.i + 1][this.j])
      neighbors[Right] = grid[this.i + 1][this.j]
    if (grid[this.i][this.j + 1]) neighbors[Bottom] = grid[this.i][this.j + 1]
    if (grid[this.i - 1] && grid[this.i - 1][this.j])
      neighbors[Left] = grid[this.i - 1][this.j]

    if (selectedDirection === undefined) return neighbors
    else return neighbors[selectedDirection]
  }
